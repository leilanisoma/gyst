...
Not yet done: alpha-transparent cutouts for the three object icons (currently circle-cropped from a plain-white background, per the recap's "Not done yet" section); the companion character redesign; and per-room dressing/content-layout polish (9D-2..9D-5 still open below) now that each room has its background.

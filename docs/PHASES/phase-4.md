...
- **Stage funnel counts "ever reached (or skipped past)," not "currently at" or "has an explicit event for."** A rejected application that made it to `interview` still counts toward the `interview` milestone, and an application whose stage dropdown jumped straight from `saved` to `interview` — with no `to_stage: "applied"` event ever recorded — still counts toward `applied`. `computeStageFunnel` (revised 2026-07-23) ranks `FUNNEL_STAGES` by pipeline order and, for each milestone, counts applications with any `application_events` row whose `to_stage` rank is at or past it, rather than requiring an exact `to_stage` match. `computeWeeklyGoalProgress`/`computeApplicationsPerWeek` use the same "reached or later" rule (via the shared `firstReachedDates` helper), each application counted once in the week it first qualified.

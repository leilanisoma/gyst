...
- **The stage funnel is a live snapshot ("currently at"), not a cumulative "ever reached" funnel — that's `computeApplicationsPerWeek`/`computeWeeklyGoalProgress`'s job instead.** `computeStageFunnel` (revised again 2026-07-23, same day) counts each application once, by its current `stage`, across every `APPLICATION_BOARD_STAGES` value including `rejected` (its own bar, not excluded — under a current-stage view it's just wherever some applications sit, not a milestone to progress past). The "ever reached (or skipped past)" logic — crediting an application that jumped the stage dropdown straight from `saved` to `interview`, with no `to_stage: "applied"` event ever recorded — still lives in `firstReachedDates`/`PROGRESSION_RANK`, but now only backs the two week-based metrics: `computeApplicationsPerWeek` (applications per week, oldest first) and `computeWeeklyGoalProgress` (this week's count against the goal), each counting an application once in the week/period it first qualified.

...
- **`google/oauth.ts` gained an optional `redirectUri` override on `buildGoogleAuthUrl`/`exchangeCodeForTokens`**, defaulting to the existing `GOOGLE_REDIRECT_URI` behavior (zero change for Calendar's call sites) — this is what lets Gmail reuse the exact same OAuth2 exchange logic with its own redirect URI/callback route instead of duplicating ~50 lines of near-identical `fetch` calls. `refreshGoogleAccessToken` needed no change (Google's refresh_token grant doesn't take a redirect_uri).
- **New env var `GMAIL_REDIRECT_URI`, reusing `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`** — those identify the registered OAuth app, not the account, so the same Google Cloud Console client works for both connections; only the callback path differs, and it needs its own authorized-redirect-URI entry in that same client's console config (an external step only the user can do — documented in `.env.example`).
...
- **`/api/gmail/connect` and `/api/gmail/callback` mirror the Google routes exactly**, with their own `gm_oauth_state` cookie (distinct name from Calendar's `g_oauth_state` — two independent OAuth flows, can't share a state cookie) and `getProfile()` (Gmail API `users.getProfile`) in place of `listCalendars().find(primary)` for capturing `account_email`.
...
- **Not live-verified against a real Gmail account** — completing an actual Google OAuth consent screen needs an interactive browser session this sandbox doesn't have, the same limitation Phase 4/5/6 flagged for their own auth flows. Verified via `tsc`/`eslint` and the fact that the OAuth exchange/refresh code paths are the exact same, already-live-verified Calendar code (just parameterized with a different redirect URI) — task 7.1/7.2 didn't touch that logic's actual behavior, only its inputs.

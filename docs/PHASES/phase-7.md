...
- **`extractGmailItemsFromMessage` (`src/lib/gmail/extract.ts`) is gated exactly like syllabus extraction** (`src/lib/syllabus/extract.ts`): a `null` client short-circuits to the same "no provider is configured" error before touching the Gmail API or the DB. This is no longer inert — Gemini was wired in via `docs/DECISIONS/0002-gemini-ai-provider.md`, and Gmail extraction specifically now runs on Groq via `getGmailAIClient()` (`docs/DECISIONS/0003-groq-for-gmail-extraction.md`), falling back to the global Gemini client if `GROQ_API_KEY` is unset.
...
- **2026-07-23: Groq capacity errors fall back to Gemini with the full body, rather than truncating it.** Real failure: a long quoted-thread/newsletter email's full plain-text body (`findPlainTextBody` has no length limit) requested ~24k tokens in one Groq call against the org's 8000-tokens/minute on-demand cap and got rejected outright (413 `rate_limit_exceeded`). A first pass capped the body at a fixed character length before sending it to any provider, but that risks silently dropping real actionable content (a deadline stated late in a long email) — reverted same-day once flagged. The actual fix (`runExtraction` in `extract.ts`) tries Groq first as before, and on a 413/429 (`GroqApiError`, now exported from `src/ai/providers/groq.ts`) retries once on the global Gemini client (`getAIClient()`) with the exact same, untruncated `messageText` — Gemini's much larger context window absorbs it. Any other error (or no fallback provider configured) returns `{ok: false, error}` instead of throwing, so one bad message no longer aborts the rest of a sync run's loop (`src/lib/gmail/sync.ts`) — a latent pre-existing bug this same fix closes, since nothing previously caught an extraction-call exception at all. Covered by tests: capacity-error fallback receives an identical full body on both providers, no-fallback-available surfaces a clean error, and a non-capacity error does *not* trigger the Gemini retry.

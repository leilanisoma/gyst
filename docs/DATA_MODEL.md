...
| `integrations` | **Implemented.** Provider (`google`, `canvas` since Phase 6, `gmail` since Phase 7), status, granted scopes (text[]), account email, `settings` jsonb (`fixed_calendar_ids`, `gyst_calendar_id`, per-calendar `sync_tokens`), last sync, error state. One row per provider per user — Gmail gets its own provider value (not `google`) since it's a different Google account than the one connected for Calendar. RLS: `auth.uid() = user_id`. See `supabase/migrations/20260712070001_google_calendar_and_notifications.sql`. |
| `oauth_tokens` | **Implemented.** Encrypted (AES-256-GCM), server-only provider tokens (`google`, `gmail` since Phase 7); never read from client code, RLS scoped by owner as defense in depth. |
| `sync_runs` | **Implemented.** Provider (`google`, `canvas` since Phase 6, `gmail` since Phase 7), start/end, aggregate counts, error, retry count — an audit log. Per-calendar incremental cursors live in `integrations.settings.sync_tokens` instead (Google issues one syncToken per calendar, not per account); see `docs/PHASES/phase-3.md` Notes. |
...
| Table | Purpose |
|---|---|
| `gmail_items` | **Implemented.** Extraction review queue — kind (`interview`/`confirmation`/`deadline`/`action`/`other`), title, AES-256-GCM-encrypted `excerpt_encrypted` (never the full message body), `date_at`, `requested_action`, confidence, `confirmed`, sticky `dismissed_at`, `expires_at` (task 7.8 retention, purged by `/api/cron/purge-gmail-items`). RLS: `auth.uid() = user_id`. |
| `gmail_processed_messages` | **Implemented.** Just `gmail_message_id` + `processed_at`, no content — lets a re-sync skip a message it already extracted from without re-fetching it to check. RLS: `auth.uid() = user_id`. |
| `gmail_drafts` | **Implemented.** Draft-only reply/follow-up tracking (task 7.7) — `status` (`proposed` only in GYST / `created` pushed to the real Gmail drafts folder via the API), `gmail_draft_id` once created. This app never calls a send endpoint; the user always sends from Gmail itself. RLS: `auth.uid() = user_id`. |

`integrations`/`oauth_tokens`/`sync_runs` also accept `provider = 'gmail'` (task 7.1) as a connection distinct from Calendar's `'google'` row, since Gmail is confirmed to be a different Google account for this user.

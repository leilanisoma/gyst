...
- Prompts as versioned files under `src/ai/prompts/` — once a provider exists to use them. AI-assisted job scoring dimensions, draft generation, discovery classification, syllabus item extraction, and Gmail item extraction (Phase 7 task 7.4) are all stubbed/deterministic or inert for this until then.

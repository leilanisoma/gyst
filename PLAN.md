...
**2026-07-16 update:** descoped. Ishani declined the Apple Developer Program enrollment this requires, so the native companion described above will not be built. Apple Watch/HealthKit data (steps, sleep, active energy, workouts) is entered manually in the webapp instead — see Phase 9B in §15 and `docs/PHASES/phase-9.md`. The built-and-tested device-pairing/token-exchange code was removed rather than left unused.

**2026-07-23 update:** `resting_heart_rate` dropped from `health_daily_summaries` — no longer tracked. Cycle tracking (`cycle_observations`) gained fertility-monitor hormone readings (LH, E3G, PdG, FSH, tested every other day) and a period on/off flag, entered via a manual per-day form alongside the existing CSV import.
...
- [x] **9D-4** - Recruiting room: office/desk theming. Done 2026-07-21, bundled with real functional additions Ishani asked for in the same pass: a `DeadlineTimeline` replacing the old ClosingSoon/FollowUpsDue text lists, ghosted-application detection (60+ days with no reply), a weekly application-goal meter, and upgraded dashboard charts (ran the dataviz skill for real, which surfaced a genuine pre-existing gap - this app's `--chart-1..5` tokens fail the six-checks palette validator in both light and dark mode; worked around it for this feature, flagged the fix as a separate follow-up). Same-day follow-up: the static "More" panel became `RoomSideTabs`, a desktop-only right-docked tab rail that expands leftward on click and closes on click-away (reused as-is for Wellness's "More" panel too); Pipeline's tab is now a big searchable table with stat tiles up top (`min(820px, 80vw)`, not a full-screen takeover), and the Kanban board option was dropped entirely in favor of one spreadsheet-style table with a stage dropdown per row. Full account in `docs/PHASES/phase-9d.md`.
- [x] **9D-5** - School room: study-nook theming. Done 2026-07-23, same split-panel template as Wellness/Recruiting - star panel holds the review queues (assessments/milestones/syllabus items) + upcoming assessments, "More" tab rail holds Canvas sync/Courses/Syllabus/Actual time log, plus a new Tasks tab embedding the existing drag-and-drop `TaskBoard` filtered to `area = 'school'` (resolves the "fuller task board" open question - `/tasks` stays the cross-area board). New `GrowthPlant` ambient visual fed by school task completion rate. Verified live via Playwright against a real authenticated session (`docs/PHASES/phase-9d.md` for the full account).
